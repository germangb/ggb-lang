...
fn codegen_statement(routines: &[Routine],
    ...
    match statement {
        ...
        Ld { source, destination, } => write!(output, "{}={}", dest(destination), src(source))?,
        Inc { source, destination, } => write!(output, "{}=({} as u8).wrapping_add(1u8)", dest(destination), src(source))?,
        Dec { source, destination, } => write!(output, "{}=({} as u8).wrapping_sub(1u8)", dest(destination), src(source))?,
        Add { destination, left, right, } => write!(output, "{}=({} as u8).wrapping_add({} as u8)", dest(destination), src(left), src(right))?,
        Sub { destination, left, right, } => write!(output, "{}=({} as u8).wrapping_sub({} as u8)", dest(destination), src(left), src(right))?,
        And { destination, left, right, } => write!(output, "{}={}&{}", dest(destination), src(left), src(right))?,
        Xor { destination, left, right, } => write!(output, "{}={}^{}", dest(destination), src(left), src(right))?,
        Or { destination, left, right, } => write!(output, "{}={}|{}", dest(destination), src(left), src(right))?,
        LeftShift { destination, left, right, } => write!(output, "{}={}<<{}", dest(destination), src(left), src(right))?,
        RightShift { destination, left, right, } => write!(output, "{}={}>>{}", dest(destination), src(left), src(right))?,
        Mul { destination, left, right, } => write!(output, "{}={}.wrapping_mul({})", dest(destination), src(left), src(right))?,
        Div { destination, left, right, } => write!(output, "{}={}.wrapping_div({})", dest(destination), src(left), src(right))?,
        Rem { destination, left, right, } => write!(output, "{}={}.wrapping_rem({})", dest(destination), src(left), src(right))?,
        Eq { destination, left, right, } => write!(output, "{}=if {}=={}{{1}}else{{0}}", dest(destination), src(left), src(right))?,
        NotEq { destination, left, right, } => write!(output, "{}=if {}!={}{{1}}else{{0}}", dest(destination), src(left), src(right))?,
        Greater { destination, left, right, } => write!(output, "{}=if {}>{}{{1}}else{{0}}", dest(destination), src(left), src(right))?,
        GreaterEq { destination, left, right, } => write!(output, "{}=if {}>={}{{1}}else{{0}}", dest(destination), src(left), src(right))?,
        Less { destination, left, right, } => write!(output, "{}=if {}<{}{{1}}else{{0}}", dest(destination), src(left), src(right))?,
        LessEq { destination, left, right, } => write!(output, "{}=if {}<={}{{1}}else{{0}}", dest(destination), src(left), src(right))?,
